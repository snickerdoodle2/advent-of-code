...
struct QueueItem(u32, usize, usize, Direction);
...
fn walk(map: &Map, x: usize, y: usize) -> Option<u32> {
    let mut heap = BinaryHeap::new();
    heap.push(Reverse(QueueItem(0, x, y, Direction::East)));
    let mut cache = HashMap::new();

    while let Some(Reverse(QueueItem(cost, x, y, dir))) = heap.pop() {
        let item = map[y][x];
        if item == 'E' {
            return Some(cost);
        }
        ...
        heap.push(Reverse(QueueItem(cost + 1000, x, y, dir.next_clockwise())));
        ...
        heap.push(Reverse(QueueItem(cost + 1, x, y, dir)));
    }
